...
const ChromaGrid = ({ items, className = '', radius = 300, damping = 0.45, fadeOut = 0.6, ease = 'power3.out' }) => {
  ...
  return (
    <div
      ...
      className={`relative w-full flex flex-wrap justify-center items-center gap-6 ${className}`}
      style={{
        '--r': `${radius}px`,
        '--x': '50%',
        '--y': '50%'
      ...
      {data.map((c, i) => (
        ...
        >
          ...
          <div className="relative z-10 flex-1 p-[10px] box-border">
            <img src={c.image} alt={c.title} loading="lazy" className="w-full h-[250px] object-cover rounded-[14px]" />
          </div>
          <footer className="relative z-10 p-4 text-white font-sans grid grid-cols-[1fr_auto] gap-x-3 gap-y-1">
            <h3 className="m-0 text-[1.05rem] font-semibold">{c.title}</h3>
            {c.handle && <span className="text-[0.95rem] opacity-70 text-right">{c.handle}</span>}
            <p className="m-0 text-[0.85rem] opacity-80">{c.subtitle}</p>
            {c.location && <span className="text-[0.85rem] opacity-80 text-right">{c.location}</span>}
          </footer>
        </article>
      ))}
    </div>
